...
function log.notify(scope, level, verbose, str, ...)
    if not verbose and _G.Ftag.nvim.config ~= nil and not _G.Ftag.nvim.config.debug then
        return
    end
    ...
    vim.notify(
        string.format("[ftag.nvim.nvim@%s] %s", scope, string.format(str, ...)),
        level,
        { title = "ftag.nvim.nvim" }
    )
end
...
function log.warn_deprecation(options)
    ...
    if uses_deprecated_option then
        ...
        log.notify(
            ...
            vim.log.levels.WARN,
            true,
            "use `:h Ftag.nvim.options` to read more."
        )
    end
end
