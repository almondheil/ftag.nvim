...
local state = require("ftag.nvim.state")
...
function main.toggle(scope)
    if state.get_enabled(state) then
        log.debug(scope, "ftag.nvim is now disabled!")

        return main.disable(scope)
    end

    log.debug(scope, "ftag.nvim is now enabled!")
...
function main.enable(scope)
    if state.get_enabled(state) then
        log.debug(scope, "ftag.nvim is already enabled")
    ...
    -- saves the state globally to `_G.Ftag.nvim.state`
    state.save(state)
end
...
function main.disable(scope)
    if not state.get_enabled(state) then
        log.debug(scope, "ftag.nvim is already disabled")
